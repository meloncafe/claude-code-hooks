...
def check_limits_and_warn(current_tokens: int, total_tokens: int, limits: Dict[str, Any]) -> bool:
    ...
    if state == 'critical' and display.get('show_on_critical', True):
        if format_type == 'silent':
            ...
            label = "세션" if check_type == "session" else "전체"
            msg = f"🚨 {label} {check_value:,}/{critical_threshold:,}"
            sys.stderr.write(f"\n{msg}\n")
            sys.stderr.flush()
        else:  # detailed
            label = "세션 토큰" if check_type == "session" else "전체 누적 토큰"
            msg = f"\n🚨 CRITICAL: {label} {check_value:,} / {critical_threshold:,} 초과!\n"
            sys.stderr.write(msg)
            sys.stderr.flush()
            console.print(f"[red bold]{msg.strip()}[/red bold]")

        if check_limits.get('auto_stop', False):
            console.print("[red]작업 중단됨 (auto_stop enabled)[/red]")
            should_stop = True
    ...
    try:
        ...
        if should_stop:
            ...
            try:
                ...
                if config_path.exists():
                    ...
                    if save_on_critical:
                        if auto_compact_enabled:
                            ...
                            sys.stderr.write("\n" + "="*70 + "\n")
                            sys.stderr.write("🔄 CRITICAL: 자동 컨텍스트 압축 시작\n")
                            sys.stderr.write("="*70 + "\n")
                            sys.stderr.write("압축이 백그라운드에서 실행 중입니다...\n")
                            sys.stderr.write("완료 후 /clear 또는 새 세션을 시작하세요\n")
                            sys.stderr.write("="*70 + "\n\n")
                            sys.stderr.flush()
                        else:
                            # Manual instruction
                ...
                console.print(f"[yellow]⚠ save_on_critical 처리 실패: {e}[/yellow]")
...
def reset(
    ...
    # Success message
    console.print(Panel(
        f"[green]✓ 토큰 사용량 리셋 완료[/green]\n\n"
        f"리셋 전: {old_total:,} tokens ({old_sessions} sessions)\n"
        f"리셋 후: 0 tokens\n"
        f"리셋 횟수: {usage['reset_count']}",
        title="Token Reset",
        border_style="green"
    ))
...
def status():
    ...
    if check_value >= check_critical:
        console.print(f"[red bold]🚨 CRITICAL: {check_type} 토큰 {check_value:,} / {check_critical:,} 초과![/red bold]")
    elif check_value >= check_warning:
        percentage = check_value * 100 // check_critical
        console.print(f"[yellow]⚠️  WARNING: {check_type} 토큰 {check_value:,} / {check_critical:,} ({percentage}%)[/yellow]")
    else:
        console.print(f"[green]✓ 정상: {check_type} 토큰 사용량 안정적[/green]")
