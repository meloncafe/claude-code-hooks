...
명령어 및 도구 사용을 제한하고 대안을 제시하는 훅

Hook Event: PreToolUse
Purpose: 특정 명령어나 도구 사용을 차단하고 더 나은 대안을 제안
"""
import json
import re
import sys
...
class CommandRestrictor:
    ...
    def _match_pattern(self, text: str, pattern_config: Dict[str, str]) -> bool:
        ...
        텍스트가 패턴과 일치하는지 확인 (정규식 또는 와일드카드).
    ...
    def is_settings_local_allowed(self, command: str) -> bool:
        ...
        명령어가 settings.local.json 허용 패턴에 있는지 확인.
    ...
    try:
        ...
        if tool_name == "Bash":
            ...
            if warning:
                ...
                # warning_only_mode면 allow, 아니면 ask
        ...
        # 모든 체크 통과 - 명시적으로 allow 응답
        add_breadcrumb("All checks passed", category="validation")
        logger.log_end(success=True, action="allowed")

        # allowlist에 있거나 제한사항이 없는 경우 명시적으로 allow 반환
        output = create_hook_output("allow", "명령어 실행이 허용됨")
        print(json.dumps(output))
