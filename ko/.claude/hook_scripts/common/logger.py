...
class HookLogger:
    ...
    def __init__(self, script_name: str):
        """
        Args:
            script_name: 스크립트 이름 (예: "session-start", "auto-compact")
        """
...
def rotate_logs(log_dir: Optional[Path] = None, max_files: Optional[int] = None) -> None:
    ...
    Args:
        log_dir: 로그 디렉토리 (None이면 자동 탐지)
        max_files: 각 스크립트별 최대 보관 파일 수 (None이면 설정에서 로드)
    """
    # 설정 로드
